...
<!--                            <div class="icon" @click="deleteWeibo(item)" style="cursor:pointer;"><img src="../../styles/删除.png" height="20" width="20"/><span>删除</span></div>-->
            ...
            <div class="right" style="width: 48%">
                <h2>我的评论</h2>
                <div class="tweet" v-for="item in userComments">
                    <el-card :body-style="{ padding: '0px' }">
                        <div class="input-head">
                            <div class="tweet-user-info">
                                <div class="tweet-avatar">
                            ...
                            <div class="icon" @click="deleteComment(item)" style="cursor:pointer;margin-top: 20px"><img src="../../styles/删除.png" height="20" width="20" /></div>
                            <div class="weiquan" v-if="item.tag==1"><img src="../../styles/weiquan.png" height="50px" width="auto"/></div>
                        </div>
                        <div class="tweet-content" style="font-size: 17px;">
                            {{item.content}}
                        </div>
                        <div  v-if="item.weibo!=null" :body-style="{ padding: '0px' }" style="background-color:#F7F7F7;padding-bottom: 5px" >
                            <div class="input" >
                                <div class="tweet-user-info">
                                    <div class="tweet-user-name" style="margin-left: 14px">
                                        <div style="font-size: 14px;font-weight: bold;">@{{item.weibo.name}}</div>
                                        <div style="font-size: 11px;color: #99a2aa;">{{item.weibo.sendTime}}</div>
                                    </div>
                                </div>
                            </div>
                            <div class="tweet-content" style="font-size: 14px;">
    ...
    export default {
        ...
        methods:{
            deleteComment(item){
                ...
                params.append('commentId', item.commentId)
                this.$axios.post('/delComment',params).then(successResponse =>{
                    if (successResponse.data.code===200){
                        this.$message.success('删除成功');
                        this.getData(localStorage.getItem("id"))
                        this.getUserComments(localStorage.getItem("id"))
                    }
                    else{
            ...
            getData(id){
                this.$axios.get('/getLikeWeibo',{ params: {
                        userId:id,
